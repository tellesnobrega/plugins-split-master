...
def get_cluster(version='5.13.0'):
    cluster = c_u.get_fake_cluster(plugin_name='CDH', hadoop_version=version)
    return cluster


class EdpEngineTestV5130(sahara_base.SaharaTestCase):

    def setUp(self):
        super(EdpEngineTestV5130, self).setUp()
        pb.setup_plugins()
    ...
    @mock.patch('sahara.plugins.cdh.v5_13_0.edp_engine.EdpOozieEngine')
